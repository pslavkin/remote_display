...
//-----------------------------------------------------------------
void Send_Event(unsigned int Event,State** Machine)	//escribe un nuevo dato a la cola...
{
...
void Atomic_Send_Event(unsigned int Event, State** Machine) {Atomic(Send_Event(Event,Machine));} 
//-------------------------------------------------------------------------------------
void Insert_Event(unsigned int Event, State** Machine)	//escribe un nuevo dato a la cola...
{
...
//-----------------------------
void Set_Event_Listener(unsigned char Index,unsigned int Event, State** Machine)
{
...
{
 unsigned char i;
 for(i=0;i<MAX_EVENT_LISTENER;i++)  Set_Event_Listener(i,0,Empty_State_Machine);
}
//------------------------------------------
unsigned char Search_Empty_Event_Listener(void)
{
 unsigned char i;
 for(i=0;i<MAX_EVENT_LISTENER && Event_Listener_Buffer[i].Machine!=Empty_State_Machine;i++);	//recorre todas las entradas fijas en busca de un lugar vacio...
...
void Send_Event2Event_Listener(unsigned int Search_Event,unsigned int Sending_Event)
{
 ...
 for(i=0;i<MAX_EVENT_LISTENER;i++)
  if(Event_Listener_Buffer[i].Machine!=Empty_State_Machine && Event_Listener_Buffer[i].Event==Search_Event)
     Atomic_Send_Event(Sending_Event,Event_Listener_Buffer[i].Machine);
}
//----------------------------------------------------------------------
void Add_Event_Listener(unsigned int Event,State** Machine)
{
...
//---------------------
void Free_Event_Listener(unsigned int Event,State** Machine)
{
 unsigned char i;
 for(i=0;i<MAX_EVENT_LISTENER;i++)
  if(Event_Listener_Buffer[i].Machine==Machine && Event_Listener_Buffer[i].Event==Event)
    Set_Event_Listener(i,ANY_Event,Empty_State_Machine);
}
//---------------------
void Free_All_Event_Listener(State** Machine)
{
 unsigned char i;
 for(i=0;i<MAX_EVENT_LISTENER;i++)
