...
char* Int2Bcd(char* Bcd,unsigned int Bin) //convierte un Long (OJO menor a 99999, si se requiere mayor rango solamente se deberan agregar valores al array de const de arriba y reservar lugar para mas BCD claro) en su BCD y coloca cada BCD en 1 byte entero (no se puso 2 por bytes porque fue menos eficiente en espacio de flash y despues para procesarlos con los bits de paridad pesaba mas, el calsico Flash Vs Ram)...
{
   unsigned char i;                                      //contador auxiliar...
...
char* Int2_3Digit_Bcd(char* Bcd,unsigned int Bin)  //convierte un Long (OJO menor a 999, si se requiere mayor rango solamente se deberan agregar valores al array de const de arriba y reservar lugar para mas BCD claro) en su BCD y coloca cada BCD en 1 byte entero (no se puso 2 por bytes porque fue menos eficiente en espacio de flash y despues para procesarlos con los bits de paridad pesaba mas, el calsico Flash Vs Ram)...
{
 unsigned char i;                                     //contador auxiliar...
 unsigned int Aux;                                       //es un long auxiliar para acordarse del valor de una resta de longs...
 for (i=0;i<3;i++)   
...
char* Int2_4Digit_Bcd(char* Bcd,unsigned int Bin)  //convierte un Long (OJO menor a 999, si se requiere mayor rango solamente se deberan agregar valores al array de const de arriba y reservar lugar para mas BCD claro) en su BCD y coloca cada BCD en 1 byte entero (no se puso 2 por bytes porque fue menos eficiente en espacio de flash y despues para procesarlos con los bits de paridad pesaba mas, el calsico Flash Vs Ram)...
{
 unsigned char i;                                     //contador auxiliar...
 unsigned int Aux;                                       //es un long auxiliar para acordarse del valor de una resta de longs...
 for (i=0;i<4;i++)   
...
char* Char2Bcd(char* Bcd, unsigned char Bin)                //convierte un Char a BCD...
{
...
char* Char2_2Digit_Bcd(char* Bcd, unsigned char Bin)                 //convierte un Char a BCD...
{
 unsigned char Aux,i;
  for(i=0;i<2;i++)
...
unsigned long Dec_Bcd2Long(char* Bcd)
{
 unsigned char i;
...
unsigned int Dec_Bcd2Int(char* Bcd)
{
 unsigned char i;
...
unsigned char Dec_Bcd2Char(char* Bcd)
{
 unsigned char i;
...
char* Char2Hex_Bcd(char* Bcd,unsigned char Bin) //convierte un Long (OJO menor a 99999, si se requiere mayor rango solamente se deberan agregar valores al array de const de arriba y reservar lugar para mas BCD claro) en su BCD y coloca cada BCD en 1 byte entero (no se puso 2 por bytes porque fue menos eficiente en espacio de flash y despues para procesarlos con los bits de paridad pesaba mas, el calsico Flash Vs Ram)...
{
 unsigned char i;                                     //contador auxiliar...
 unsigned int Aux;                                       //es un long auxiliar para acordarse del valor de una resta de longs...
 for (i=0;i<2;i++)   
...
char* Int2Hex_Bcd(char* Bcd,uint16_t Bin)
{
   char Aux=Bin>>9;
...
//----------------------------------------------------------------
char* Replace_Zero2Space(char* Buf,unsigned char Length)
{
   unsigned char i;
